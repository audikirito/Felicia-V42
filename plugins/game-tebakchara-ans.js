...
export async function before(m) {
    ...
    if (!(id in this.tebakchara))
        return conn.sendButton(m.chat, '*_Soal Itu Telah Berakhir_*', author, null, buttontebakchara, m)
    if (m.quoted.id == this.tebakchara[id][0].id) {
        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text)
        if (isSurrender) {
            clearTimeout(this.tebakchara[id][3])
            delete this.tebakchara[id]
            return conn.sendButton(m.chat, '*Kok Menyerah?*', author, null, buttontebakchara, m)
        }
