...
let ps = groupMetadata.participants.map(v => v.id)
    ...
    */
    ...
    await conn.sendButton(m.chat, caption, wm, null, [['𝗝𝗢𝗗𝗢𝗛𝗡𝗬𝗔', `${usedPrefix}jodohnya`],['𝗝𝗢𝗗𝗢𝗛𝗞𝗨', `${usedPrefix}jodohku`]], m, { mentions: conn.parseMention(caption) })
    }
	if(isNaN(text)) {
  ...
  try {
    ...
    if(global.db.data.users[m.sender].pasangan != "" && global.db.data.users[global.db.data.users[m.sender].pasangan].pasangan == m.sender && global.db.data.users[m.sender].pasangan != user){
      conn.reply(m.chat, `Kamu Sudah Berpacaran Dengan *@${global.db.data.users[m.sender].pasangan.split('@')[0]}*\n\nSilahkan *.putus* Dulu Untuk Menembak *@${user.split('@')[0]}*\n\n*Jangan Pernah Selingkuh...*`, m , { contextInfo: { mentionedJid: [user, global.db.data.users[m.sender].pasangan]}})
      ...
      if (pacar){
        ...
        conn.reply(m.chat, `*@${user.split('@')[0]}* Sudah Berpacaran Dengan *@${pacar.split('@')[0]}*\n*Silahkan Cari Pasangan Lain, Jangan Nikung*`, m , { contextInfo: { mentionedJid: [user, pacar]}})
      }else{
        global.db.data.users[m.sender].pasangan = user
        conn.reply(m.chat, `Kamu: *${await ktnmbk.getRandom()}*\n\nAnda Baru Saja Mengajak @${user.split('@')[0]} Berpacaran\n\n*Silahkan Menunggu Jawaban Darinya*\n\nKetik *${usedPrefix}terima @Tag* Untuk Menerima\n*${usedPrefix}tolak @Tag* Untuk Menolak`, m , { contextInfo: { mentionedJid: [user]}})
      }
    }else if (global.db.data.users[user].pasangan == m.sender){
      global.db.data.users[m.sender].pasangan = user
      conn.reply(m.chat, `Selamat Anda Resmi Berpacaran Dengan *@${user.split('@')[0]}*\n\n*Semoga Langgeng Dan Bahagia Selalu ❤️*\n*Dont Forget Ewe ❤️*`, m , { contextInfo: { mentionedJid: [user]}})
    }else {
      global.db.data.users[m.sender].pasangan = user
